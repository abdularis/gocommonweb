...
const ossSignedURLExpire = 60 * 60 * 24 // 1 Day
...
func (s *storageAlibabaOSS) URL(objectPath string, isObjectPublic bool) (string, error) {
	if objectPath == "" {
		return "", nil
	}
	...
	return s.bucket.SignURL(objectPath, oss.HTTPGet, ossSignedURLExpire)
}
